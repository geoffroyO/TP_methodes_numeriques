...
function plot_error(W, Refx, Ux, Refy, Uy, Errx, Erry, Y, X) // On plot les différents résultats 
    fig = gcf()
    Wm = min(W); WM = max(W)
    Uxm = min(Ux); UxM = max(Ux)
    Uym = min(Uy); UyM = max(Uy)
    Rxm = min(Refx); RxM = max(Refx)
    Rym = min(Refy); RyM = max(Refy)
    Exm = min(Errx); ExM = max(Errx)
    Eym = min(Erry); EyM = max(Erry)
    clf();
    xset("colormap", jetcolormap(64))
    subplot(341)
    colorbar(Wm, WM)
    plot3d1(Y, X, W)
    title('$\omega$','fontsize',5)
    subplot(342)
    colorbar(Rxm, RxM)
    plot3d1(Y, X, Refx)
    title('$\mathbf{u}_x^*$','fontsize',5)
    subplot(343)
    colorbar(Uxm, UxM)
    plot3d1(Y, X, Ux)
    title('$\mathbf{u}_x$','fontsize',5)
    ...
    colorbar(Exm, ExM)
    plot3d1(Y, X, Errx)
    title('$\|\mathbf{u}_x - \mathbf{u}_x^* \|$','fontsize',5)
    subplot(345)
    colorbar(Rym, RyM)
    plot3d1(Y, X, Refy)
    title('$\mathbf{u}_y^*$','fontsize',5)
    subplot(346)
    colorbar(Uym, UyM)
    plot3d(Y, X, Uy)
    title('$\mathbf{u}_y$','fontsize',5)
    ...
    colorbar(Eym, EyM)
    plot3d1(Y, X, Erry)
    title('$\|\mathbf{u}_y - \mathbf{u}_y^* \|$','fontsize',5)
