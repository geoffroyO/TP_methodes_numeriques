...
function [freqs]=fftfreq(N, L)
    k = modulo(N, 2)
    if k == 0 then
        freqs = 2*%i*%pi * cat(2, linspace(0, N/2 -1, N/2 ), linspace(-N/2, -1, N/2))/L;
    else  
        freqs = 2*%i*%pi * cat(2, linspace(0, (N-1)/2, (N+1)/2), linspace(-(N-1)/2, -1, (N-1)/2))/L;
    end
endfunction
    ...
    for p=1:Ny
        for q=1:Nx
            if p==1 & q==1 then 
                ...
                u_x_hat(p,q) = (-ky(p)/(kx(q)**2 + ky(p)**2))*w_hat(p,q);
