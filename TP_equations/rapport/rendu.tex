...
\vdots \\
\Phi((N_x-1)dw, kdt)
\end{pmatrix}$ et $T \begin{pmatrix}\Phi(0, (k+1)dt) \\
\vdots \\
\Phi(ndx, (k+1)dt) \\
\vdots \\
\Phi((N_x-1)dx, (k+1)dt)
...
\includegraphics[width=0.9\linewidth, height=0.3\textheight]{isocontours_1_20.png}
\captionof{figure}{Isocontours à $t=1.2$}
\end{minipage}
\begin{minipage}{.5\textwidth}
\includegraphics[width=0.9\linewidth, height=0.3\textheight]{isocontours0_800000.png}
\captionof{figure}{Isocontours à $t=0.8$}
\end{minipage}
\vspace{0.8cm}
\\
Pour $t=0.12$ les lignes de niveau sont plus resserrées, ce qui signifie que le champ scalaire de la vorticité varie rapidement lorsqu'on se déplace orthogonalement aux isocontours. Pour $t=0.8$, on a des isocontours plus espacés, la vorticité varie moins rapidement. Physiquement, on assiste à la "création" du tourbillon.
\\
\begin{center}
\includegraphics[scale=0.13]{ite_0240.png}
\captionof{figure}{$\omega$, $u_{x}(x,y)$, $u_{y}(x,y)$ pour $\nu=10^{-4}$ et $\rho=30$}
\end{center}
On a 240 itérations pour cette simulation.
\newpage
\textbf{Question 14:}
\\
\begin{minipage}{.5\textwidth}
\includegraphics[width=0.9\linewidth, height=0.3\textheight]{14_isocontours_12.png}
\captionof{figure}{Isocontours à $t=1.2$}
\end{minipage}
\begin{minipage}{.5\textwidth}
\includegraphics[width=0.9\linewidth, height=0.3\textheight]{14_isocontours_08.png}
\captionof{figure}{Isocontours à $t=0.8$}
\end{minipage}
\vspace{0.8cm}
\\
Pour ces paramètres, on a un tourbillon plus important que celui de la question précédente. On a du itérer 294 fois, ce qui rend cette configuration plus difficile à simuler que la première.
\begin{center}
\includegraphics[scale=0.13]{14_ite_0294.png}
\captionof{figure}{$\omega$, $u_{x}(x,y)$, $u_{y}(x,y)$ pour $\nu=5\cdot10^{-5}$ et $\rho=100$}
\end{center}
